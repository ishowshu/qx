...
if (url.includes("/aos/perception/publicTravel/beforeNavi")) {
  ...
  // 搜索结果地图左下角小横幅
  if (obj?.data?.modules?.poiDetailNewBeltV2) {
    obj.data.modules.poiDetailNewBeltV2 = {};
  }
  
  // 搜索结果底部评论打分
  if (obj?.data?.modules?.combineReviews?.data?.write_comment) {
    obj.data.modules.combineReviews.data.write_comment = {};
    obj.data.modules.combineReviews.data.nav_bar_write_comment = {};
  }
